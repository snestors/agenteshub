...
- **Restart correcto**: `bin/safe-restart.sh` (no `systemctl restart` salvo emergencia)
...
# ─── 1) Build frontend (si frontend/ cambió o se bumpeó la versión) ─────────
...
| Hay cambios sin versionar | Bumpear `VERSION` + `frontend/package.json` + nueva entrada en `RELEASE_NOTES.md` (mismo commit) y entonces correr el flujo |
...
- Asumir que `safe-restart` promueve `bin/agenthub.next`
