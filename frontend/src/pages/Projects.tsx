...
import { Plus, FolderKanban, TerminalSquare } from "lucide-react";
import { api, FALLBACK_ENGINES, type EngineDef, type Project, type ProjectSession } from "@/lib/api";
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 min-h-0 p-4 grid grid-cols-[310px_1fr] gap-4">
        ...
        <HudPanel
          title={current ? `project chat · ${current.name}` : "project chat"}
          sub={current ? `topic project_session:${current.id}` : "sin sesión"}
          accent="magenta"
          className="min-h-0"
        >
          {error && <ErrorBox msg={error} />}
