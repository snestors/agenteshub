...
func (e *CodexEngine) codexAgenthubMCPArgs(opts RunOpts) []string {
	...
	pairs := [][2]string{
		...
		{"args", `["mcp"]`},
		{"env.AGENTHUB_DB_PATH", strconv.Quote(absPathStr(cfg.DBPath))},
		...
		{"env.AGENTHUB_UPLOAD_DIR", strconv.Quote(absPathStr(cfg.UploadDir))},
		{"env.AGENTHUB_WA_MEDIA_DIR", strconv.Quote(absPathStr(cfg.WAMediaDir))},
