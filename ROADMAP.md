...
| Fecha | Cambio |
| --- | --- |
| 2026-04-27 | Fases E + F + G implementadas (backend + types frontend): **G** — `internal/projects/phase_models.go` lee `<project>/.claude/phase-models.yaml` opcional para overridear el `RoleDefault` del SDD por proyecto; cableado en propose/design/tasks/apply/verify. **F** — tabla `skill_improvements`, repo, endpoints `GET/POST /api/skills/improvements` + `POST /api/skills/improvements/{id}/resolve` para el flujo propose-and-approve de auto-mejora; smoke E2E pasado. **E** — endpoint `GET /api/projects/{id}/docs` (lista canónica) y `GET /api/projects/{id}/docs/{doc}` (contenido de `claude/specs/design/agents/release`); `projectDocsApi` y `skillImprovementsApi` en frontend listos para construir las tabs sin backend nuevo. |
| 2026-04-27 | Fase D implementada (backend + types): 4 templates iniciales en `~/.claude/project-templates/` (astro-frontend, go-backend-gin, python-fastapi, cf-pages-static) con agentes, skills, services_initial, claude_md_seed y spec_md_seed. Paquete `internal/projecttemplates` parsea YAML + cachea 30s. Endpoints `GET /api/project-templates`, `GET /api/project-templates/{name}`, `POST /api/projects/{id}/apply-template` (renderiza placeholders {{name}}, escribe CLAUDE.md/SPECS.md/services.yaml respetando archivos no vacíos salvo `overwrite: true`). UI de galería pendiente para próxima iteración. |
| 2026-04-27 | Fase C implementada (versión local): tabla `skills`, repo, paquete `internal/skillsync` que escanea `~/.claude/skills/` y persiste cada `SKILL.md` (frontmatter YAML + body) como row con source `local`. Endpoints `GET /api/skills` (filtrable por source) y `POST /api/skills/sync`. Pantalla `/skills` con tabla, filtro por source, modal detalle con markdown renderizado, y botón "sync now". Pull desde repos git remotos queda como TODO documentado. |
